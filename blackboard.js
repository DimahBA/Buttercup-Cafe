        ...
        setTimeout(() => {
            ...
            this.followupsDiv.innerHTML = followups.map(followup => `
                <button onclick="blackboardQA.selectQuestion('${followup.id}')"
                        class="flex items-center gap-2 md:gap-3 hover:scale-105 transition-transform duration-200 cursor-pointer text-left group">
                    <img src="./static/PixelArt/tinyFlower.png" class="w-4 md:w-6 group-hover:scale-110 transition-transform" alt="">
                    <span class="text-white font-serif text-lg md:text-xl group-hover:text-gray-200">${followup.text}</span>
                </button>
            `).join('');
